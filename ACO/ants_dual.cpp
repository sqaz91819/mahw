...
namespace ANT
{
    ...
    double pheXdis1[SIZE][SIZE] = {{gamma}};
    double pheXdis[SIZE][SIZE] = {{1}};
    double p1[SIZE][SIZE] = {{gamma}};
    ...
    pair<vector<int>, double> construct_ant1()
    {
        uniform_int_distribution<int> distribution = uniform_int_distribution<int>(0, 50);
        unordered_map<int, int> unused(51);
        vector<int> s;
        ...
        for (int i = 0; i < SIZE - 1; i++)
        {
            ...
            for (auto it = unused.begin(); it != unused.end(); ++it, ++index)
            {
                CDF[index] = (pheXdis1[start][it->second] / total) + CDF[index - 1];
            }
    ...
    void init_trail()
    {
        ...
        for (int k = 0; k < SIZE; k++)
        {
            for (int l = 0; l < SIZE; l++)
            {
                ...
                pheXdis1[k][l] = p1[k][l] * pow(inverse_dis[k][l], alpha);
            }
        }
    }
    ...
    auto ACO_main(int runtimes, int iteration, int popsize)
    {
        ...
        for (int i = 0; i < runtimes; i++)
        {
            ...
            for (int j = 0; j < iteration; j++)
            {
                ...
                vector<pair<vector<int>, double>> Population1;
