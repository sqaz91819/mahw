...
namespace GA_PMX{
    ...
    */
    ...
    pair<Solution, Solution> cross_over(Solution Pa, Solution Pb){
        ...
        // copy original segment from c1 to c2.
        for(int i = c1; i <= c2; i++){
            n_a[i] = a[i];
            n_b[i] = b[i];
        }
        ...
        int i = c2 + 1;
        int candidate_a = c2 + 1, candidate_b = c2 + 1;
        while(i != c1) {
            int j = c1;
            while( j <= c2) {
                if(b[candidate_a] == n_a[j]) {
                    candidate_a += 1;
                    if(candidate_a == coordination.size())
                        candidate_a = 0;
                    j = c1;
                }
                else j++;
            ...
            n_a[i] = b[candidate_a];
            candidate_a++;
            if(candidate_a == coordination.size()) candidate_a = 0;

            j = c1;
            while( j <= c2) {
                if(a[candidate_b] == n_b[j]) {
                    candidate_b += 1;
                    if(candidate_b == coordination.size())
                        candidate_b = 0;
                    j = c1;
                }
                else j++;
            ...
            candidate_b++;
            if(candidate_b == coordination.size()) candidate_b = 0;

            i++;
            if(i == coordination.size()) i = 0;
    ...
    */
    ...
    vector<double> OX_main(int iteration=2000, int popsize=50) throw(){
        double best = 99999;
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < runtimes; i++) {
        vector<double> record = GA_PMX::OX_main(iteration, popsize);
    ...
    GA_PMX::run_output( "OX.txt", run_record);
    return 0;
}